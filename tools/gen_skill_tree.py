...
def build_plan(manifest: dict):
    ...
    role is one of: 'core', 'claude', 'gpt'. NOT 'codex' -- see the comment at the end
    of the loop below: codex adapters have no legacy source to migrate FROM.
    ...
    for s in manifest["skills"]:
        ...
        # NO codex record, deliberately. This plan maps the RETIRED legacy `.claude/`
        # tree onto the canonical skills/ tree, and `_record` asserts every entry has a
        # `.claude/`-rooted legacy source. Codex adapters are new work authored directly
        # in the canonical tree (Phase CP Step 4 onward) -- they never existed in the
        # legacy layout, so the manifest gives them no `migration.legacy_codex` key and
        # inventing one would fabricate provenance for a file that was never migrated.
    return plan
...
def build_inventory(manifest: dict) -> dict:
    ...
    for s in manifest["skills"]:
        ...
        if status == "provider-native":
            rec["exclusion"] = {
                ...
                "core": False,
                "gpt": False,
                # Provider-native is CLAUDE-ONLY, so the exclusion covers codex for
                # the same reason it covers gpt. Stated rather than implied: the
                # builder excludes native skills from every non-claude profile, and an
                # exclusion record that named only gpt would under-report that.
                "codex": False,
            }
        skills.append(rec)
    counts = {
        ...
        "provider_native": sum(1 for s in skills
                               if s["status"] == "provider-native"),
        # Per-provider adapter tally, read off the booleans above so this count and
        # the per-skill records cannot disagree. Mirrors the manifest's counts["codex"]
        # (config/skill-manifest.json, owned by tools/gen_manifest.py); the two are
        # cross-checked by tests/package-integrity/test_skill_tree.py.
    ...
    return {
        "note": (
            ...
            "Portable skills carry a neutral core plus Claude and GPT adapters, and "
            "optionally a Codex adapter (additive third provider); provider-native "
            "skills carry only a Claude adapter and an explicit exclusion record."
        ),
