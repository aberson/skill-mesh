...
- This wrapper maps no isolated fresh-context agent primitive: this core requires no isolated arm, so the whole review runs in this session, with the same ordering and the same evidence requirement -- claims about the plan are validated against the real files before they are reported, never inferred.
