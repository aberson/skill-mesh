...
- Codex has no Artifact tool: the plan and the bootstrapped project instruction file -- CLAUDE.md or AGENTS.md, whichever the core selects (see the Instruction-file contract in plan-init/core.md, [`../core.md`](../core.md)) -- are written as FILES exactly as the core orders, and this wrapper restates no part of that contract. The plan-redline hook fires through Codex's named-skill dispatch; its codex adapter publishes the proposal as a standalone HTML file, so the closing line's proposal locator is that repository-relative path rather than an artifact URL. If the hook fails, report `proposal skipped (<reason>)` and continue -- the hook never blocks, exactly as the core states.
