...
- Fresh-context dispatch is a runtime HOST capability, not a provider-wide Codex constant: the verdict leg is capability-conditioned on BOTH conjuncts the core states, never on one. The core requires a fresh-context VISION-CAPABLE task that VIEWS each image and cross-checks it against the read-back, so a host qualifies only when it passes the separate build-step agent-isolation contract AND its child is separately proven image-capable to the SAME standard that contract sets -- proven, never inferred from a model or provider name -- so a text-only child that passes the isolation probe does NOT qualify. A host that passes both may map the independent vision-judge onto a fresh sibling child. This core documents NO single-context fallback for that dispatch -- the orchestrator that drove the browser never renders the verdict -- so on a host that fails EITHER conjunct, halt visibly with `required_tool_missing`, naming the vision-judge dispatch as the core step that could not run. Never view the screenshots and grade your own driving, and never auto-PASS through the missing judge.
