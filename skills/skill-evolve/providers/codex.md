...
- This core scores variants through the HOST WORKFLOW PRIMITIVE, not through isolated agent dispatch: its "parallel by default" principle runs the shared score_skill.workflow.js script once for all variants, and passing a no-history agent probe establishes nothing about whether that primitive can execute. This wrapper maps no host workflow primitive, so this core's own halt contract governs: halt visibly (core halt #2, "Workflow unavailable") with `required_tool_missing`, naming the scoring-workflow dispatch as the core step that could not run, rather than falling back to a single self-grading agent -- the exact defect this skill was rebuilt to avoid. Never serialize mutator, render, and grader roles into this session, and never let anything grade output it produced.
