...
- This core delegates single-trial scoring to the HOST WORKFLOW PRIMITIVE, not to isolated agent dispatch: Step D runs the shared score_skill.workflow.js script, which owns the render/grade split and the trial-median aggregation internally, and passing a no-history agent probe establishes nothing about whether that primitive can execute. This wrapper maps no host workflow primitive, and this core documents NO single-context fallback for the split: keeping render and grade as separate agents is non-negotiable, and no task grades its own output. Halt visibly with `required_tool_missing`, naming the score-loop workflow dispatch as the core step that could not run. Never self-grade a render, and never serialize the brainstorm arm, render tasks, and grader tasks into this session.
