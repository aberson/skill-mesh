...
- This wrapper maps no Workflow primitive, so it does not select the session path: run the core's documented CLI fallback (`uv run goblin suggest <project>`, which shells generator and judge calls to `claude -p` subprocesses with a ThreadPoolExecutor per candidate) through Codex's shell tool -- judge independence and parallelism live in those subprocesses, unchanged. Where the `claude` CLI or its OAuth token is absent from the environment, halt visibly with `required_tool_missing` naming the suggest rail; never generate-and-judge candidates inside this one session.
