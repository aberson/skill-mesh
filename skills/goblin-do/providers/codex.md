...
- This wrapper maps no Workflow primitive, so it does not select the session path: the execute rail runs the core's documented CLI fallback (`uv run goblin do <id>`, which shells the `/build-step` dispatch to a `claude -p` subprocess) through Codex's shell tool. Where the `claude` CLI or its OAuth token is absent from the environment, halt visibly with `required_tool_missing` naming the execute rail -- never degrade to an unreviewed inline edit of the target.
