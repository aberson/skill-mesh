<!-- BAD OUTPUT — DEFECT: --reviewers passed an invalid value ("static") not in the three accepted values (code | runtime | full); echoed back without error -->
<!-- Violates SKILL.md lines 647-655: "--reviewers code|runtime|full ... Three values are valid; the default is code." Unknown values MUST not be silently accepted. -->

# review-deep: NEEDS-WORK

Invocation: `--prompt "wire judge sampling into the propose flow per plan step 15" --diff <staged> --plan-step documentation/toybox-plan.md:15 --reviewers static --output-dir .review-deep/`
