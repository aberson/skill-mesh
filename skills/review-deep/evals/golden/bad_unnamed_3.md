<!-- BAD OUTPUT — DEFECT: invocation header omits required --output-dir (default .review-deep/) AND --reviewers default; one of the 10 documented flags missing from the invocation echo -->
<!-- Violates SKILL.md lines 22-36: Arguments table documents 10 flags; the invocation echo must reflect resolved values including the documented defaults (--reviewers default code, --output-dir default .review-deep/) -->

# review-deep: NEEDS-WORK

Invocation: `--prompt "wire judge sampling into the propose flow per plan step 15" --diff <staged> --plan-step documentation/toybox-plan.md:15`
...
- **tests/test_judge_sample.py:1-58** (test-quality): The test file imports `schedule_judge_sample` directly and exercises 4 sampling-rate paths in isolation; it would pass even if the helper were never invoked by production.
