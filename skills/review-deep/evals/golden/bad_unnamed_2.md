<!-- BAD OUTPUT — DEFECT: frontmatter description omits "plan-conformance" lens (only four lenses named) -->
<!-- Violates SKILL.md line 3: "Five-lens code review (correctness, bugs, tests, style, plan-conformance) ..." — description must name all five lenses -->

# review-deep [4-lens code review with anti-pattern catalog]: NEEDS-WORK
...
- **tests/test_judge_sample.py:1-58** (test-quality): The test file imports `schedule_judge_sample` directly and exercises 4 sampling-rate paths in isolation; it would pass even if the helper were never invoked by production. Recommend rewriting one test to drive `_do_propose`.
