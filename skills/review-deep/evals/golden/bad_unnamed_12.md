<!-- BAD OUTPUT — DEFECT: Block finding tags an anti-pattern name that is NOT in the seven-entry Tier-1 catalog ("missing-error-handling" — invented name) -->
<!-- Violates SKILL.md lines 316-408: exactly seven Tier-1 anti-pattern entries (silent-wiring, producer-consumer-drift, codifying-test-diff, silent-fallthrough-in-loop, silent-fallthrough-in-hot-path, duplicate-shape-constants, create-table-without-migration); anti_pattern field MUST be one of these (plus reserved scope-boundary-deferral marker) -->
...
- **src/toybox/judge_sample.py:1** (bugs, anti-pattern: missing-error-handling): The diff adds `schedule_judge_sample(propose_ctx)` and a direct-import unit test, but grep across `src/toybox/` finds zero production references; `_do_propose` in `src/toybox/propose.py` was not modified, so the production effect is zero judge calls forever. Fix: add an integration test that drives `_do_propose` end-to-end.
...
- **documentation/toybox-plan.md:Step 15** (plan-conformance): Done-when clause `judge runs on >0 sampled proposes per session` is unmet by the diff — no call site in `_do_propose` or any other production caller reaches `schedule_judge_sample`.
