<!-- BAD OUTPUT — DEFECT: header reads only DEFERRED-TO-UAT when both NEEDS-WORK (Block findings) and DEFERRED-TO-UAT apply (markdown header must name both; sidecar's aggregated_verdict.result must be NEEDS-WORK) -->
<!-- Violates SKILL.md lines 619-626: "if BOTH apply on the same diff ... the markdown summary names BOTH in the header line (# review-deep: NEEDS-WORK + DEFERRED-TO-UAT), but the JSON sidecar uses aggregated_verdict.result == NEEDS-WORK (the harder verdict wins for gating purposes)" -->
...
- **src/toybox/judge_sample.py:1** (bugs, anti-pattern: silent-wiring): The diff adds `schedule_judge_sample(propose_ctx)` and a direct-import unit test, but grep across `src/toybox/` finds zero production references.
