...
- This wrapper maps no isolated fresh-context dispatch for the per-lens reviewer fan-out -- the accepted known gap recorded as decision DS-D3 in `documentation/descope-2026-09.md`, not a claim about what a Codex host can do -- and this core documents NO single-context fallback for that fan-out: the producer never grades itself, and lens independence comes from context isolation. Halt visibly with `required_tool_missing`, naming the lens dispatch as the core step that could not run. Never serialize the lenses into this session, never self-grade the diff, and never emit a verdict the fan-out did not produce.
...
- On timeout, rate limit, provider 5xx, parse failure, or missing required adapter, return the router reason code and consume at most the shared cross-cloud retry allowance. Never silently omit a core gate, and never downgrade or drop a finding because the fan-out that would have produced it did not run.
