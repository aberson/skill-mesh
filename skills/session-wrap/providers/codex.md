...
- Invoke `task-handoff` by loading its installed skill/core in this session and executing it with the real filesystem/Git tools where core calls for it. A named-skill API is not required; missing identity or filesystem/helper capabilities still fail visibly. The task-state file contract stays owned by the schema document the core cites -- this wrapper restates none of it.
