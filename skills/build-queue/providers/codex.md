...
- Cross-skill routing: invoke `/plan-expedite` and `/build-phase` through Codex's named-skill dispatch, sequentially and never in parallel, exactly as the core orders. `/build-phase` is capability-conditioned here: it applies its own three capability gates (fresh no-history children, opaque parent key retention, a caller-scoped parent-only verdict service) and halts `required_tool_missing` before a code step when the active host does not pass them; that halt is a PARK, not a queue abort -- record it, file the park issue with `--body-file`, and move to the next item exactly as the core's park procedure states.
