...
| Phase IS (#143) | BUILDING (2026-08-26) — 9 of 11 landed, 8 certified DONE. Step 107 (#151) is DONE: sentinel `0`, repo-root gate `1341 passed, 1 skipped`, baseline updated at `719e622`; its four concurrent-review documentation defects are fixed at `52d44c9`. Step 108 (#152) is LANDED / CERTIFICATION PENDING: build/install PASS; repaired exact bytes `5B3FC466…995F8` have two independent high-0/medium-0 reviews, 25/25 PowerShell-bearing fences parse, and 278 package tests pass, but the stable repo-root DONE gate remains pending. Step 108P (#162) is **BLOCKED BEFORE IMPLEMENTATION** on #152 and the #153 route choice; it owns all containment code. Step 109 (#153) remains operator-only and **BLOCKED BEFORE GRADING**, with every behavioral cell blank. | The operator must choose on #153 between a new core-supported UAT mode (then rebuild/reinstall/reverify) and a deliberate plan amendment accepting narrower named-skill subsection overrides. Step 108P then implements and certifies the route's one-handle roots, three-stage receipt chain, OS-admitted native launcher/helper, strict JSON, handle-coupled reads, kernel I/O rail, exact environment/hook/policy/process closure, exact core/Codex/Claude evidence, portable attestations, and safe cleanup, followed by its full gate. Step 109 only invokes those artifacts and records observations. Do not start a host session until #152 is DONE, the route is recorded, and #162 is complete. Authority: `documentation/instruction-file-symmetry-plan.md` and `documentation/findings/instruction-file-symmetry-uat.md` § 2. |
...
The repaired transcript's exact bytes (`5B3FC466…995F8`) have two independent high-0/medium-0
reviews, all 25 PowerShell-bearing fences parse, and 278 package tests pass; its stable repo-root
DONE gate remains pending. Its historical
...
install/reverify from those exact bytes. Those roots must bind both final paths and file ID from one
open handle, with UNC/SUBST/8.3 aliases, nesting, and case-mismatched allowlist targets rejected;
