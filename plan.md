...
main. This operator-authorized detour creates stable utility code under `C:\Users\abero\prod` without
changing the frozen Phase IS UAT or claiming that `--reviewers deep` is restored. Phase RD #178 is
...
| Phase PROD — Stable production utility toolchain | AUTHORIZED / BUILDING — `documentation/production-toolchain-separation-plan.md`; no external production directory or active-profile mutation has occurred yet | Run automated Steps 1–4 through fresh Terra build-step contexts using the operator-authorized `--reviewers code` bootstrap exception, then execute attended Step 5 from the retained candidate and complete Step 6. This detour pauses but does not satisfy Phase RD; deep-dependent builds remain blocked until #178–#181 pass. |
...
wholesale. Phase PROD's only permitted active-home mutation is its attended Step 5 using the exact
Step-4 retained distribution through the existing installer and inspector; this narrow, explicit
exception does not authorize any certificate, policy, boot, SDK/WDK, driver, frozen-UAT, or Phase IS
artifact change. It also does not waive `--reviewers deep` for consumer projects.
...
The approved C2E overlay is `documentation/phase-is-disposable-c2n-c4-environment-plan.md`. It adds no
...
or real host profiles except through Phase RD Step 4's separate target-specific approval after its
disposable rehearsal. The operator authorized source preparation on 2026-08-31. C2E Steps 1–3 may
start only after `documentation/phase-rd-review-deep-restoration-plan.md` restores and activates
mandatory deep review; a provider/license/cost/retention choice remains required at Step 4, and provisioning
...
External consumer boundary: sibling repository `../pta_finance` remains untouched and its
issue #27 / `documentation/treasurer-slides-plan.md` Step 14 remains paused. It may resume at Step 14
only when all four Phase RD statuses and issues are `DONE`/closed, #177 is closed, #165 remains open
with the adapter progress recorded, `main` is clean and synchronized, the final root gate passes,
the sealed Phase IS artifacts remain byte-identical, and the linked Step 4 issue contains a remotely
reread `PhaseRdActivationSealV1` proving the reviewed active Codex closure, installed calibration,
three helper exit-0 checks, and fresh supported capability verdict. Its required `--reviewers deep
--isolation worktree` lane is not weakened, bypassed, or substituted.
...
do not mix its 58/55 catalog change into the current frozen Phase IS candidate.
...
**Within parked Phase CP, the only remaining action after Phase IS completes is operator Step M3
(#132).** M4 (#133) is PASS as of 2026-08-20:
...
prose contradictions. At `596af1b`, one passed and the other reported 0 high / 4 medium additional
stale-contract contradictions. At `e4cdad0`, one passed and the other reported 0 high / 2 medium
ordering cycles; at `6e71905`, one passed and the other reported 0 high / 1 medium on contradictory
real-profile transport writes. At `ed3cce0`, both independently reported the same sole 0 high / 1
medium defect: "54/54 files emitted" conflated skill and file counts. Those ten plus two pre-freeze
phase-plan ordering contradictions are repaired. The next candidate is Git blob
`bd1c9205b80c468dcbb4ef64da5b1532e1cfba06`, raw SHA-256
...
lifetime guardian must retain each root, their common creation-parent handle, and the real-profile handle
from creation through disposition, remeasuring both final paths, `FILE_ID_INFO`, and link count from
the same live objects; UNC/SUBST/8.3 aliases, nesting, and case-mismatched allowlist targets are rejected;
every read/mutation/snapshot/launch/parse/grade uses a fresh receipt-pinned self-contained bundle as
...
managed policy cannot refresh before its exact startup-consumed snapshot is bound ahead of any
hook/process, contained trust and fail-closed mode-specific hook activation, a preventively
image/argv-gated descendant process tree, isolated config/auth and
background-lifecycle controls, complete native Base/Profile/attribution/core-read binding, and an
exact asynchronously captured Claude import event, portable redacted evidence, and no-follow
disposition-recording cleanup. No
...
3. **Park-and-pivot — SUPERSEDED 2026-08-26 by an in-session operator instruction.** Round 5 had
   ...
   **Updated 2026-08-26 — the deferred track's two prerequisites are now BOTH already satisfied, so
   ...
   meant to gate. The fresh detached run at `d4c88ee` completed with sentinel `0` and `1341 passed,
   1 skipped`; only after comparison against the captured **1335 passed / 1 skipped** figure was the
