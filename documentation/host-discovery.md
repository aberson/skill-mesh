...
runtime injection even when a native `.copilot/skills` tree is absent (see the
...
`.copilot/skills` root for GPT, the `.claude/skills` root for Claude — never from
the fact that a model produced a plausible answer.

## Host-native skill discovery — the discovery-root table
...
provider to a provider-specific root:

| Provider | Host | Native discovery location |
|---|---|---|
...
- **GitHub Copilot CLI discovers each skill at `<install-home>/.copilot/skills/<skill>/SKILL.md`.**

This table matches the discovery-location table in the repository
[`README.md`](../README.md) ("Providers & installation") and the installer's own
target map (`install-skill-mesh.ps1`: `claude -> .claude/skills`,
`gpt -> .copilot/skills`). The two roots are provider-specific and are never
swapped: a Claude install never populates `.copilot/skills`, and a GPT install
never populates `.claude/skills`. See the per-host guides
[`providers/claude.md`](providers/claude.md) and
[`providers/gpt.md`](providers/gpt.md).

The GPT root (`.copilot/skills`) is the assumption every later cutover step depends
on; the cutover plan proves it against a live GitHub Copilot CLI session before any
inspection or migration tooling is built on it (cutover plan §6, "Host binding is
the normal path").
...
- **Neither file enumerates or embeds skill implementations.** A workspace
  ...
  (`.claude/skills`, `.copilot/skills`) above.
- The installer proves this structurally: it writes skill trees **only** under the
  discovery roots (`<Home>/.claude/skills/<skill>/…`, `<Home>/.copilot/skills/<skill>/…`)
...
- A host that runtime-injects a `CLAUDE.md` (for example a Copilot CLI that exposes
  ...
  that a `.copilot/skills` profile is installed (cutover plan §2, §8).
...
- **Host-native binding (primary).** When a host discovers a skill through its own
  scan of `.claude/skills` / `.copilot/skills`, the bound adapter is already
...
| Which skill tree was discovered? | the captured `SKILL.md` path (`.claude/skills` for Claude, `.copilot/skills` for GPT) | the model's output |
...
profile is proven only by the second row's `.copilot/skills` path capture. Keeping
these mechanisms distinct — and never interchangeable — is what prevents a running
model from being mistaken for a correctly installed profile.
...
- [`../config/skill-manifest.json`](../config/skill-manifest.json) — the single source of truth for which skills are published.
