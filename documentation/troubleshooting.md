...
support. Step 156 must prove the fresh installed
...
inconclusive probe cannot be retried into success, replaced by self-review or
silently switched to another host/model. Runtime/full review remains unsupported
by this mapping; in-repository calibration is unchanged, not an installed command.

Missing `scripts/aggregate.py`, `scripts/lint_prepass.sh`, `scripts/README.md` or
`<loaded-package>/config/model-tier-map.md` indicates stale or incomplete generated bytes. Rebuild
and install normally; never repair consumer files by hand. If ownership hashes
or foreign files conflict, preserve them and leave qualification incomplete.
The audit is unsigned; only the separate enclosing build-step service can
authenticate advancement. `UNCERTAIN` and `NEEDS-CLARIFICATION` stop before the
unchanged reducer because it cannot safely represent those states today.

The separately qualified Claude route remains independent and requires its own
fresh preflight. Phase CD does not change consumer deep-review flags or retry history.
