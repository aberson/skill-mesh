...
| Best-effort containment gate | The static gate misses documented blind-spot expression shapes, and the content-identity half of the invariant is convention-guarded until Step 47b's tripwire lands | Runtime protections (`Resolve-HomeTarget` containment, `Assert-OurBytesAtTarget` content identity) are the consumer-machine controls, not the static gate; blind spots are documented in the gate module itself; Step 47's confirming review asks the content-identity question explicitly; decision §D5's trigger re-escalates to an AST-based check on the first live false negative |
