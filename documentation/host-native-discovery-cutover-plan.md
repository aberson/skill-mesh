...
- The current Copilot CLI runtime can inject the consumer workspace's
  ...
  `AGENTS.md` or an installed GPT skill tree. That behavior is host integration,
  not proof that native GPT discovery is installed correctly.
...
- `documentation/migration.md` now records Step 41 as superseded by this plan.
  The live absence of an installed `.github/skills` GPT tree confirms that GPT
  host-native cutover is not complete.
...
- **Backup directory:** an operator-selected absolute directory outside
  `.claude/skills`, `.claude/skills-gpt`, `.github/skills`, and any pre-retarget
  `.copilot/skills`. `-Apply`
...
### Step 49-50 host-trace amendment (2026-08-09)

This amendment controls the pending Step 49 and Step 50 acceptance runs and
supersedes their `ACCEPTANCE PROBE` append/readback/revert clauses. It does not
rewrite the historical Step 43 and Step 45 evidence.

Current Claude Code correctly treats a dynamically appended instruction that
asks it to emit a path and stop as untrusted prompt injection. Do not weaken or
bypass that safeguard, and do not append any runtime instruction to an
installed `SKILL.md`. Instead, retain host-generated identity records while the
generated bytes remain equal to `backup-manifest.json`'s `installed_files`
hashes:

- **Claude Code:** run a fresh `claude --setting-sources project` session from
  the consumer home. Its native session JSONL must record the session `cwd`, a
  `Skill` invocation for the representative skill, the tool-supplied `Base
  directory for this skill:` equal to
  `<home>/.claude/skills/<skill>`, the generated wrapper's `Profile: claude`,
  and `attributionSkill=<skill>`. These are host records, not model claims.
- **GitHub Copilot CLI:** with `OPENAI_API_KEY` unset, run
  `copilot -C <home> skill list --json`. The single enabled record for the
  representative portable skill must have `source: "project"` and `path`
  exactly `<home>/.github/skills/<skill>`. Its description must equal the
  staged GPT wrapper's frontmatter description. Then invoke that same skill in
  a no-write host session; the invocation proves the discovered entry is
  executable, while the CLI JSON is the authoritative loaded-path record.

Both methods capture the provider profile and exact discovery root directly
from the host. Recheck the two representative installed-file hashes after the
host observations; a mismatch is a failure. No pre-image, probe revert, or
probe-induced rollback hazard exists under this method.
...
|---|---|---|---|
| `tools/install-skill-mesh.ps1` | extend | Expose migration-safe integration without weakening marker/ledger ownership rules | Read installer: provider roots are `.claude/skills` (Claude) and `.github/skills` (GPT, post-Step-44); foreign files are refused unless `-Force`; ledger is an index, marker is authority |
...
| `documentation/providers/gpt.md` | modify | Explain the real Copilot roots (`.github/skills` install target), `AGENTS.md`, and why current runtime injection is not native installation | Current guide documents target discovery but not the absent-live-install failure mode |
...
| `coding-root/.claude/skills`, `.claude/skills-gpt`, `.claude/lib/skill-router.ps1`, `.github/skills` (plus any pre-retarget `.copilot/skills`) | live-substrate operator step only | Replace legacy active implementation with generated provider-bound installs | Live inspection found ordinary legacy directories, v1.2 router, and no installed GPT tree |
...
| `legacy_skills_gpt` | object (same shape as a `profiles` entry) | Per-skill eligibility classification of the legacy `.claude/skills-gpt` tree, kept SEPARATE from `profiles` (fixed to `{claude, gpt}`) and from `legacy_shadows` (path strings only) so an unmanifested skill present only in `.claude/skills-gpt` — e.g. `goblin-sweep`'s GPT core — classifies `consumer-only` there and NEVER `managed` (added in Step 46) |
...
| 49 Full host-native acceptance | **operator** | DONE (2026-08-09; #62 closed) |
| 50 Cut over the live coding-root consumer | **operator** | DONE (2026-08-09; #63 closed) |

Steps 49 and 50 are `Type: operator` by design: they required acceptance against real
Claude Code and GitHub Copilot CLI hosts. No test in this repository stood in for that
evidence (§9's static/operator split), so a green suite was necessary but not
sufficient. Both steps were accepted on 2026-08-09: Step 49 recorded clean temporary-home
acceptance and rollback; Step 50 completed the parked-work handshake and a coding-root-owned
live cutover commit. The retained Step 50 backup remains outside the repository.

Before acceptance, Step 49's **mechanical** half was rehearsed end to end against throwaway homes
...
49's own Done-when that left a home MIXED and unrecoverable. The subsequent real-host
acceptance is recorded in Steps 49 and 50 below.
...
- **Depends on:** none
- **Status:** DONE (2026-08-02) — **with one criterion since falsified.** The GPT clause above ("GPT discovery resolves `.copilot/skills`") was satisfied as written when this step shipped, but Step 43 (#58) disproved the underlying assumption against a live Copilot CLI and Step 44 (#66) corrected the shipped artifacts to `.github/skills` + YAML frontmatter. The Done-when text is left verbatim as the historical record of what was accepted; it is NOT a current statement of behavior. The other four clauses still hold.
...
- **Depends on:** 44
- **Status:** DONE (2026-08-03) — **PASS: both-profile collision resolves determinately, GPT profile wins, stable.** Installed both profiles into a clean temp home (`s45home`; Claude → `.claude/skills`, GPT → `.github/skills`), appended the §2 acceptance probe to BOTH `plan-review/SKILL.md` copies, and ran a real GitHub Copilot CLI **v1.0.77** session rooted there (host model Claude Sonnet, `OPENAI_API_KEY` unset). (1) `copilot skill list` **dedups by name** — `plan-review` is listed exactly once, and the listed description is the GPT copy's ("Review a plan for gaps, missing pieces, unresolved decisions, and risks…"), NOT the Claude entry-point description ("Claude provider entry point for plan-review…"); every portable skill shows its GPT-style description. (2) Invocation, run **twice**, stably resolved to the `.github/skills` copy: run 2's tool trace shows Copilot glob `**/plan-review/SKILL.md` → **2 files found** → **Read `.github\skills\plan-review\SKILL.md`**; run 1's PROBE printed `profile=gpt` (run 1's `…\providers\gpt.md` suffix was a model readback artifact echoing the header's `Canonical source:` line — the only file carrying the probe is `.github/skills/plan-review/SKILL.md`; no `providers/gpt.md` is installed). (3) Mechanism: `.github/skills` precedes `.claude/skills` in Copilot's discovery order (`copilot skill --help`: "Project `.github/skills/`, `.agents/skills/`, or `.claude/skills/`"), so the GPT profile is a **stable, ordering-based** dedup winner — the GPT skill is NOT shadowed by the Claude profile. **Consequence:** the migrator MAY install both profiles to a live home; NO branch to a single-shared-`SKILL.md` root is needed — Steps 46–50 proceed as planned. Claude Code independently reads `.claude/skills` for its own profile, so both hosts get their correct profile. Evidence: #67. **Separate finding (does NOT block; filed as sibling #69):** `context-slim`'s `.claude/skills` `SKILL.md` failed Copilot's YAML parse — the Claude-profile frontmatter emits `description`/`argument` UNQUOTED, so a colon in the value (`argument: Optional flags: …`) breaks strict parsers (`mapping values are not allowed in this context at line 4 column 25`); the GPT builder path double-quotes via `ConvertTo-YamlDoubleQuoted`, the Claude path does not. Bounded: context-slim was the only failure across all 50 skills, and it does not affect the collision resolution.
...
- **Issue:** #59
- **Flags:** --reviewers deep --isolation worktree
- **Files:** `tools/inspect-host-install.ps1`, `config/skill-manifest.json` (read-only eligibility source), `tests/distributions/`, `tests/distributions/legacy_install_fixtures.py`
...
- **Status:** DONE (2026-08-03), HARDENED (2026-08-04) — `tools/inspect-host-install.ps1` + `tests/distributions/test_host_inspect.py`. Read-only proven by full-tree SHA-256 before/after; text + stable `HostInstallReport` JSON (`schema_version` 1); default home-relative paths, `-AbsolutePaths` opt-in. Manifest-driven eligibility (managed/consumer-only/core-holder/foreign) with the `core:null` provider-native single-profile carve-out. Reuses `tools/skill-mesh-provenance.ps1` (dot-sourced, not forked). Deep review PASS (5 lenses: 0 high, 1 med [stale comment, fixed], lows). Merged to main (3a16d2b). The tool emits an additional top-level `legacy_skills_gpt` object (blessed in §5) to carry per-skill classification of the legacy `.claude/skills-gpt` tree without altering the fixed `profiles`/`legacy_shadows` shapes.
  - **Hardening pass (#83–#86), 2026-08-04:** a full-suite wrap found four defects in the shipped step, all fixed before Step 47 built on the report.
    ...
    - **#85 coverage gaps.** Added the `foreign` negative anchor (the four-class model had none), all three corrupt-ledger return paths, router `canonical`/`legacy`/unparseable-version classification, and the `<external>` junction sentinel — the absolute-path leak guard, previously never executed because the only junction fixture pointed inside the home.
    - **#86 fixtures discovered as live skills.** The 12 committed fixture `SKILL.md` files sat at real discovery paths, so this repository published phantom `build-phase`/`build-step`/`context-slim`/`build-observer`/`goblin-sweep` skills to anyone working in it. All shapes are now synthesized at test time by `tests/distributions/legacy_install_fixtures.py`; a `git ls-files`-enumerated gate fails if any tracked file returns to a discovery path.
...
- **Superseded status (2026-08-05, round 3):** BLOCKED — **max `/build-step` iterations exhausted (3/3). Not a test failure.** Round 3 was a genuine refactor (one choke point, one shared content-identity rule, a committed structural gate, both coverage gaps closed, #89 folded in) and it is a real improvement, but the gate rejected it too — all six lenses this time. The verdict trend is 4 → 5 → 4 Blocks, and the defect moved UP one level of abstraction each round rather than being eliminated: call site (round 1), cluster (round 2), then the abstraction itself (round 3). Round 3's three central claims are each falsified with reproduction: (a) `Resolve-HomeTarget` — THE choke point — returns `Join-HomePathLexical`'s **unresolved** output rather than the reparse-resolved path its docstring promises, because `Test-ContainedInHome` computes `Resolve-SafePath`'s result and discards it, so the ancestor-junction TOCTOU (time-of-check-to-time-of-use) window is still open while the sibling choke points (`Resolve-TxPath`, the installer's `Resolve-Contained`) do return the resolved path; (b) `tests/distributions/test_path_choke_point.py` inspects only `-Destination` and never `-LiteralPath`, so it cannot catch a regression of the very prepared-phase-backup `-LiteralPath` source defect it was built for (proven by invoking `violations_in()` on a synthetic regression), and its `$safe*` derivation rule is one-hop-deep so a two-hop alias chain defeats it; (c) a fourth null-coercion sibling survives at `New-BackupManifest:889-891`, handing a possibly-`$null` resolver result into an unguarded `Test-Path -LiteralPath` that throws in PS 5.1 and preempts the tool's own `PRECONDITION_DRIFT`/exit-2 path. Suite is green and independently re-verified at **536 passed / 3 skipped** (baseline 440; `main` separately verified at 440/3); all 16 tracked `.ps1` ASCII-only, no BOM, parsing clean. Work preserved and pushed on `build-step-1785890195` (`f795834`); nothing merged. **Keepable wins from the three rounds:** `tools/skill-mesh-discovery.ps1` (one owner for the provider→discovery-root map AND provider-slug normalisation, consumed by installer + inspector + migrator — closes a real `duplicate-shape-constants` defect and #89), ~96 new tests including the no-prior-ledger undo branch that had never executed and an `UNSAFE_LINK` junction planted after planning so only the apply-time gate can catch it, and several defects the agent's own tests caught (notably rollback reporting `rolled_back` over a home it had not restored). **Recommended next attempt:** do NOT reopen the 4,800-line diff — fix the choke point's two-line contract violation in isolation, correct the gate to inspect `-LiteralPath` and to follow `$safe*` chains transitively, then re-run that corrected gate over the current tree, which is the cheapest way to learn whether anything else remains ungated. Audit trail: issue #60 comments plus `.review-deep/2026-08-04T{19-33-40,20-45-49,23-59-45}.json` (six independent lenses per round, review-deep's deterministic `aggregate.py`, rounds 2-3 with `--prior-sidecar`).
- **Targeted fixes landed (2026-08-05, branch `5ef1045`, still UNMERGED and unreviewed):** the three-part fix recommended above was applied instead of reopening the diff. (1) `Get-ContainedHomePath` now RETURNS the canonical path and `Resolve-HomeTarget` returns that, closing the contract violation (`Test-ContainedInHome` had computed `Resolve-SafePath`'s result and discarded it with `$null =`); verified empirically that `Resolve-SafePath` canonicalizes, and safe for not-yet-existing targets since `Get-CanonicalRealPath` appends a non-existing tail to the resolved prefix. (2) The structural gate's rule is now exact rather than argument-positional — *a lexical consumer-home path may never reach the filesystem in ANY argument* — so a legitimate dist-file source stays legal while the escaped `Copy-Item -LiteralPath (Join-HomePathLexical ...)` shape is caught; the `$safe*` derivation check now follows alias chains transitively with cycles counted as ungated. Both carry red-on-garbage anchors. (3) Re-running the corrected gate over the tree found **nothing else ungated**: 56 mutating sites across 8 files, zero violations, sweep demonstrably non-vacuous. The two remaining round-3 Blocks were fixed too (`New-BackupManifest`'s null-into-`Test-Path` crash, and `Get-CreatedDirs` probing existence through the lexical joiner so an ancestor junction could decide what the ownership ledger claims). Full suite **538 passed / 3 skipped**. Step remains BLOCKED pending ONE confirming six-lens review round — a much smaller question than before, since the diff under scrutiny is three focused changes over an otherwise-unchanged tree.
- **Superseded status (2026-08-04, rounds 1-2):** stop-and-audit, halt class #3. Two `/build-step` iterations produced a working implementation (transaction engine + migrator + installer routed through it) whose FULL suite is green and independently re-verified at **521 passed / 3 skipped** (baseline 440), with all tracked `.ps1` ASCII-only/no-BOM/parsing and the installer's public parameter block byte-identical to `main`. The `--reviewers deep` gate nonetheless returned NEEDS-WORK both rounds, and the Block findings are the SAME defect shape at new sites each time: *a path that reads or mutates a consumer-home target without the containment re-check, or destroys bytes without the content-identity check, that a SIBLING branch in the same function already performs.* Five instances (iteration 1: `Invoke-ActionMutate` retire, `Invoke-ActionUndo` all branches; iteration 2: `Invoke-Apply` prepared-phase backup at `:1186`, `Invoke-ActionUndo` install-overwrite at `:972-980`, `Invoke-ActionUndo` ledger at `:982-989`) — two of them introduced BY iteration 2's own fixes. A mechanical enumeration of every mutating primitive in both tools confirms the pattern: 12 sites guarded via `$safeTarget`, while `:1186` still reaches into the home through the purely lexical `Join-HomePath` (present since the first Step 47 commit, and it corrupts the backup pre-image that rollback depends on) and `:1346` deletes a directory ungated. Per `/build-step`'s stop-and-audit rule the correct move is to STOP iterating and fix the invariant structurally rather than per call site: one choke point every consumer-home target resolves through, one shared "refuse to destroy bytes that are not ours" rule every undo branch calls, and a committed structural regression test that enumerates the mutating primitives and fails on any ungated site (same shape as this repo's existing `git ls-files`-enumerated gates). Two coverage gaps that hid this must close too: `legacy_ledger` defaults `True` in every fixture so the ledger-undo `pre_hash==null` branch has never run, and all four `UNSAFE_LINK` junction tests plant the junction before the first dry-run, so they are caught by scan-time classification and never exercise the apply-time re-check they are cited as proving. Work is preserved and pushed on branch `build-step-1785890195` (`023e2e7`); nothing merged to `main`. Audit trail: issue #60 comment, plus `.review-deep/2026-08-04T19-33-40.json` and `.review-deep/2026-08-04T20-45-49.json` (six independent lenses per round, consolidated by review-deep's own deterministic `aggregate.py`, the second run with `--prior-sidecar`). Iteration 2 did land real wins to keep: the provider-to-discovery-root map collapsed to a single owner (`tools/skill-mesh-discovery.ps1`, consumed by installer, inspector, and migrator), and its own new tests caught rollback reporting `rolled_back` over a home it had not restored.

### Step 47b: Harden the containment gate honestly (off the cutover critical path)
- **Problem:** The structural gate `tests/distributions/test_path_choke_point.py` was born as Step 47's stop-and-audit remedy and appears in no step's Done-when — a plan-orphaned artifact whose changes no plan-conformance review could bound, which is how it consumed review rounds 3–5. Its `bde8a5f` hardening was proven strictly weaker on multi-command lines (a newly-recognised alias consumes the single match slot and shadows a later ungated cmdlet), its new tests blessed the idioms they missed, and its docstring claimed a completeness ("a sixth [unguarded site] cannot appear") that a regex over PowerShell source cannot support. Step 47 reverts the gate to `5ef1045` and fixes the claim; the improvement work needs its own step with its own acceptance so it can never again block the migrator's merge.
- **Type:** code
- **Issue:** #60 (shared with Step 47 — the gate's full round history lives in that thread; mint a dedicated issue via `gh issue create` or at the first post-cutover repo-sync)
- **Flags:** --reviewers code --isolation worktree
- **Files:** `tests/distributions/test_path_choke_point.py`, `tests/distributions/` (differential corpus), `tools/migrate-legacy-install.ps1` (read-only enumeration target for the content-identity tripwire)
- **Produces:** The alias + mixed-operand hardening re-landed without the round-5 weakening, gated by a committed differential corpus of the round-5 counterexamples; a content-identity tripwire that enumerates `Invoke-ActionUndo`'s branches and fails when a branch that destroys or overwrites lacks its `Assert-OurBytesAtTarget` call; the gate's five-sites anchor test re-documented to what it actually proves (the containment instances in their historical spellings, naming the two content-identity instances it deliberately cannot represent); `SITE_EXEMPT` (the gate's per-call-site allowlist of reasons a mutating call is exempt from the containment check) caller-behavior reasons verified or re-worded; per `documentation/step-47-decomposition-decision.md` §D5 the AST (Abstract Syntax Tree)-based real check stays deferred with a named un-defer trigger (a live false negative in either half of the invariant).
- **Done when:** The differential corpus contains at minimum one multi-command line where a recognised alias precedes an ungated long-name cmdlet, the corpus is flagged by the new gate, and the `bde8a5f` gate demonstrably fails the corpus (the criterion's own red anchor — a plain no-weakening check over the real tree and the existing anchors is vacuous, since both gates flag zero there); the historical containment shapes remain caught in their historical spellings; the content-identity tripwire enumerates undo branches (never hand-listed) and goes red when an `Assert-OurBytesAtTarget` call is removed; the gate's docstring claim matches its mechanism and documents the §D3 blind-spot classes (including helper-call laundering and `LEXICAL_HOME`, the hand-maintained joiner-cmdlet name list used by the gate's alias/mixed-operand scan); red-on-garbage anchors and the sweep's non-vacuity check pass; the distribution suite passes.
- **Depends on:** 47
- **Dispatch note:** the `47b` suffix does not match `/build-phase`'s numeric step-heading walker (`### Step N:`), so a phase run over this plan dispatches 47 → 48 and does not schedule this step — intentional, since it sits off the critical path. Run it via `/build-step` directly when picked up, or renumber it into the sequence at the first post-cutover `/repo-sync`. (dev-observatory's scraper and `/repo-sync` both tolerate the suffix.)
- **Status:** PENDING — deliberately off the Step 48→50 critical path (Step 48 depends on 42 and 47 only); may land in parallel with or after the cutover steps.
...
- **Depends on:** 42, 47
- **Status:** DONE (2026-08-06, merge `f377c54`, branch commit `c2ef217`). Produced `documentation/coding-root-cutover-handoff.md` (the sequence Steps 49–50 execute) plus `tests/package-integrity/test_cutover_handoff.py`; corrected stale claims in `migration.md`, `README.md`, and `provider-neutral-skill-mesh-plan.md`, whose Step 41 now carries an explicit `**Status:** SUPERSEDED` marker. Issue #50 was left alone as the clause requires — and it turned out to be already closed as superseded (2026-08-03, against umbrella #56), so the plan text now says so rather than listing it as outstanding. Four-lens review returned **2 Blocks, both operator-safety defects, both fixed pre-merge**: (1) the handoff enumerated only `applied`/`rolled_back` as terminal, so an operator hitting exit 3 — the state its own exit-3 row routes them into — was told to run `-Resume` or `-Rollback`, and **both refuse `failed_incomplete`**, which is simultaneously unresolved (blocking a bare `-Apply`) and terminal; the migrator's own source names that advice as a defect it was corrected to remove, and the handoff now gives the real clearing action, split by which of exit 3's two meanings applied; (2) the untracked-deletion fallback destroyed the managed legacy GPT directories while claiming the retained backup and the preservation CSV were the audit trail — neither covers them (the migrator backs up only paths it mutates and never touches that tree; the CSV is built from the *non-managed* set), so an explicit copy-to-backup step now precedes the deletion. Nine Nits were also fixed, the most important being that the exit-3 safety assertion was a whole-document substring search satisfied by a *different* sentence than the one it guarded — rewriting the authoritative table cell to say to restore the backup left it green. Both re-scoped gates were mutation-proved. The gate asserts structure only: it never executes a handoff step and never asserts host acceptance passed. Full suite **574 passed / 3 skipped** (baseline 540).
...
- **Depends on:** 48
- **Status:** DONE (2026-08-09; host-trace amendment evidence accepted by the operator: clean
  release candidate, both real-host profile paths, no-write Copilot invocation, Copilot router
  dispatch without `OPENAI_API_KEY`, manifest-hash checks, and clean rollback recorded in the
  Step 49-50 operator handoff.)
...
- **Depends on:** 49
- **Status:** DONE (2026-08-09; the verified-parking gate was satisfied with an external
  bundle and clean worktrees; the live coding-root migration installed 50 Claude and 47 GPT
  generated entries, both native hosts resolved and invoked the representative `plan-review`
  profile from their own discovery root, and the legacy `-Model` router path dry-ran through the
  generated compatibility shim without `OPENAI_API_KEY`. The dedicated coding-root branch retains
  the migration backup, retires the 47 managed legacy GPT entries, preserves the consumer-only
  `goblin-sweep` tree by recorded hash, and carries the GPT `judge-ui` calibration note forward
  byte-for-byte outside the retired path. The coding-root owner committed the scoped cutover;
  unrelated consumer-only skill YAML diagnostics were observed but not changed by this step.)

#### Step 50 verified-parking amendment (2026-08-09)

The eight-hour recent-commit check is an idleness proxy, not an unconditional delay. For this
operator run, each recent coding-root program commit was identified on its named branch, every
worktree was clean, no fresh expedite-state file existed, and an external Git bundle preserved the
named refs before any consumer mutation. Those commits are parked evidence, not a competing active
session. Any unknown recent commit, dirty worktree, fresh state file, or absent external bundle
still blocks Step 50.
