...
| `documentation/coding-root-cutover-handoff.md` | modify | §9.1 future tense | `:450` ("For pending Steps 49 and 50…"), `:467` ("for the pending runs") — both pre-change; the repaired text sits at `:450-452` and `:467-468`. Must retain `operator evidence` / `parked-work handshake`, gated by `test_handoff_defers_host_acceptance_to_the_operator_steps` (`test_cutover_handoff.py:1361-1369`, measured 2026-08-11): the gated span is **all three** asserts, `:1364-1369`, not the first two |
...
- **Produces (as shipped):** corrected status text at `host-native-discovery-cutover-plan.md:677,679-683`, `coding-root-cutover-handoff.md:450-452,467-468` and `provider-neutral-skill-mesh-plan.md:326-352` (the Step 41 block, which now records Steps 49/50 as DONE while retaining all six gated tokens and tripping none of the five forbidden phrases) — all measured after the edits landed, which is why they differ from the pre-change anchors in `Produces` above; the widened gate plus its anchors and scope floor in `tests/package-integrity/test_cutover_handoff.py`; and the named in-repo decision record `documentation/step-69-doc-reconciliation-decisions.md`, which owns the KEEP-RETIRED rationale
- **Status:** DONE (2026-08-12) — merged to `main` as `f883d17`. Post-merge full-suite gate **in the MAIN checkout**: **1026 passed / 1 skipped / 0 failed** (41m36s) against the 1023/1 baseline; +3 tests, skip count unchanged. **Five developer iterations**, reviewed by the five-arm gauntlet; the step was reported BLOCKED after iteration 3 and resumed by operator decision. Block trend **0 → 0 → 1 → 0**. The one Block was not a code defect: the decision record enumerated this gate's blind spots and asserted the enumeration was COMPLETE, and a reviewer reproduced a fourth member (two ordinary triple-backtick openers each left unclosed — the everyday edit mistake) that hides a live status phrase while the delimiter count stays even. That was the **fifth instance in this phase of one shape: a claim asserting more than it can decide**, and rounds 2 and 3 had each patched the named instance and produced a new one a layer down
- **DECIDED 2026-08-12 — the fix is a CLAIM-STYLE change, not another pattern, and NOT a standing human sign-off.** Two options were weighed. **Rejected:** adopt Step 66 §7.1's tier-3 per-file sign-off to own the undecidable semantic class. Step 66's sign-off is correct for ITS problem because the failure mode there is publishing private data from a private workspace into a public repository — asymmetric, irreversible, undetectable after the fact. This step's failure mode is a stale sentence in a document. The operative test recorded is that **standing human sign-off is warranted only when the failure mode is asymmetric and irreversible**; otherwise it manufactures an obligation that is honoured for a season and then silently lapses, which is the same over-claim one layer up and harder to see because it looks like process. **Adopted:** state the closed MECHANISM (`fence_walk` decides code regions from delimiter position and pairing alone — it is intent-blind, so any markup that makes prose read as a code region exempts it), present the known blind spots as members of an **open class explicitly not claimed complete**, and add the fourth member with its own executable anchor. A later-found member is then a new example to add, not a falsified promise. No new matcher, no new bound, no sign-off. Full decision, with rejected alternatives and consequences, recorded in the operator's decision log outside this repository (`paper-trail` record `2026-08-11-prefer-guardrails-over-standing-human-sign-off-for`)
- **The de-quantification sweep had to be done twice, and the second attempt is the transferable lesson.** The first pass grepped a literal phrase across only the two files being edited, and missed a surviving assertion that (a) was written with markdown bold between the words so the literal never matched, and (b) lived in a third file. Two independent scoping errors in one grep. The second pass **enumerated every mention of the concept repo-wide and judged each**, which found the real site immediately and confirmed the ~30 other hits belonged to unrelated gates. This is the same failure the round-2 citation sweep hit from a different direction — it audited citations into files the step READ, never into files the step WROTE: **enumerate the class, do not pattern-match the instance**
- **Confirmed by measurement, not assertion:** all five named status corrections verified TRUE; the citation sweep independently confirmed complete, with 8/8 sites re-verified by reading content at the cited line, after it found **18 stale references across 8 sites** that this step's own edits had introduced plus one already wrong beforehand; Step 47b's PENDING sense intact everywhere (pre-diff count of 28 reproduced exactly); **Step 55's scope diffed byte-identical**, so the BUILD-READY Phase 8 plan had its drifted anchors corrected without any step being re-scoped; `test_cutover_handoff.py` weakened nothing (188 insertions / 0 deletions in round 1, every later hunk additive or neutral, the four named token sets byte-identical and green); the `fence_walk()` extraction proven **behaviourally byte-identical** for its other consumer across 15 hostile inputs and all 21 scanned documents; all 15 planted-defect rows reproduced with exact error-string matches; and the new fourth-member anchor verified non-vacuous by a control — the same prose unfenced yields a defect, so the anchor measures a real exemption rather than a gate that never fires. **Unplanned evidence the gate works:** the decision record's own first draft red it, having split a citation code span across a markdown line wrap so it read as prose — caught by the gate, not by review
...
- **FOLD IN #114 (added 2026-08-11) — the suite's pass/skip count is environment-dependent, and this
  step already builds the machinery to fix it.** Measured during Phase 7.5: two repo-root runs of the
  SAME commit, on the same machine, differed by 14 tests — **1026 passed / 1 skipped** from a checkout
  under the coding root versus **1012 passed / 15 skipped** from one outside it — with the collected
  total invariant at 1027. Cause: some tests probe a real, discoverable installed skill tree, and a
  checkout with no ancestor `.claude/skills/` takes their skip path. On a fresh clone or any CI machine
  with no installed consumer tree the suite reports green while silently running 14 fewer tests, which
  under this project's `count >= baseline` DONE gate (D6) is a false-green vector. This step already
  stands up a throwaway home, so it is the cheapest place to close it: synthesize the tree those tests
  need at test time (the pattern `tests/distributions/legacy_install_fixtures.py` established after the
  #83-#86 phantom-skill incident), or at minimum assert the expected skip count so a change in it reds.
  **Enumerate the affected tests by diffing the two runs' skip lists — do not hand-list them.**
