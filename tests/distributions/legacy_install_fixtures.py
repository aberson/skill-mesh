...
# A THIRD `_shared` population: skill-mesh's own generated bytes at a path no profile
# of the current distribution ships. That is the marker-only half of the two-yeses
# rule -- the only input for which "does the dist ship this path?" says no and the
# file is still ours -- and it is what the retire action exists for. Planted under
# both a live provider root (a superseded payload asset) and the RETIRED
# `.copilot/skills` root (a pre-retarget copy), because those are the two scenarios
# the rule is justified by and each reaches it through a different scan.
